      ...
      defaultShowErrors: function () {
        ...
        $.each(this.errorList, function (index, value) {
          ...
          if (bsMajorVer === 3 && bsMinorVer >= 3) {
            ...
            // Fix For Select2
            if($currentElement.hasClass("select2")){
                var $select2 = $(value.element).next("span.select2-container");

                if ($select2.data('bs.tooltip') !== undefined) {
                    $select2.data('bs.tooltip').options.title = value.message;
                } else {
                    $select2.tooltip(_this.applyTooltipOptions(value.element, value.message));
                }
                $(value.element).removeClass(_this.settings.validClass).addClass(_this.settings.errorClass);
                $select2.removeClass(_this.settings.validClass).addClass(_this.settings.errorClass).tooltip('show');
        ...
        $.each(_this.validElements(), function (index, value) {
          ...
          if($(value).hasClass("select2")){
              var $select2 = $(value).next("span.select2-container");

              $(value).removeClass(_this.settings.errorClass).addClass(_this.settings.validClass);
              $select2.removeClass(_this.settings.errorClass)
